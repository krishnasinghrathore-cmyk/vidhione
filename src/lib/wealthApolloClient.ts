...
export const getWealthGraphqlUrl = () => {
    const configured = import.meta.env.VITE_WEALTH_GRAPHQL_URL;
    if (configured) return configured;

    const base = getDefaultGraphqlUrl();
    if (/\/graphql\/?$/.test(base)) return base.replace(/\/graphql\/?$/, '/wealth/graphql');
    return `${base.replace(/\/$/, '')}/wealth/graphql`;
};

export const wealthGraphqlUrl = getWealthGraphqlUrl();
...
export const wealthApolloClient = new ApolloClient({
    link: ApolloLink.from([
        ...
        authLink,
        new HttpLink({ uri: wealthGraphqlUrl })
    ]),
