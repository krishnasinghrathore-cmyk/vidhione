...
export const getStoredAddAnotherAfterSavePreference = (): boolean => {
    const storage = resolveLocalStorage();
    if (!storage) return false;
    const normalized = normalizeAddAnotherAfterSavePreference(
        storage.getItem(PAYMENT_VOUCHER_ADD_ANOTHER_AFTER_SAVE_STORAGE_KEY)
    );
    return normalized ?? false;
};

export const persistAddAnotherAfterSavePreference = (enabled: boolean) => {
    const storage = resolveLocalStorage();
    if (!storage) return;
    storage.setItem(PAYMENT_VOUCHER_ADD_ANOTHER_AFTER_SAVE_STORAGE_KEY, enabled ? '1' : '0');
};
