...
import type { SalesReturnBookPage } from '@/lib/invoicing/api';
...
export default function BillingSalesReturnBookPage() {
    ...
    const [appliedFilters, setAppliedFilters] = useState<SalesReturnBookFilters | null>(null);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [first, setFirst] = useState(0);
    const [loading, setLoading] = useState(false);
    const [reportError, setReportError] = useState<string | null>(null);
    const [reportData, setReportData] = useState<SalesReturnBookPage | null>(null);
    ...
    const refreshButtonId = 'sales-return-book-refresh';

    useEffect(() => {
        setPageTitle('Sales Return Book');
    ...
    const displayRows = useMemo<SalesReturnBookRow[]>(() => {
        return rawRows.map((row) => ({
            rowKey: String(row.saleReturnId),
            ...
            referenceAmount: Number(row.referenceAmount ?? 0),
            referenceInvoiceIds: row.referenceInvoiceIds ?? null,
    ...
    const buildQueryVariables = (filters: SalesReturnBookFilters, offset: number, limit: number) => ({
    ...
    const fetchAllSalesReturnBookRows = useCallback(async () => {
    ...
    const { isPrinting, printRows, triggerPrint } = useReportPrint<SalesReturnBookRow>({
        rows: displayRows,
        getPrintRows: fetchAllSalesReturnBookRows
    });
    ...
    const buildFilters = (): SalesReturnBookFilters => ({
        fromDate,
        toDate,
        ledgerId,
    ...
    const loadSalesReturnBook = async (filters: SalesReturnBookFilters, offset: number, limit: number) => {
        setLoading(true);
        setReportError(null);
        try {
            const response = await invoicingApi.fetchSalesReturnBook(buildQueryVariables(filters, offset, limit));
            setReportData(response);
        } catch (err) {
            setReportError(err instanceof Error ? err.message : 'Failed to load sales return book');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRefresh = () => {
        ...
        void loadSalesReturnBook(nextFilters, 0, rowsPerPage);
    };
    ...
    const handlePageChange = (event: DataTablePageEvent) => {
        ...
        void loadSalesReturnBook(appliedFilters, event.first, event.rows);
    };
    ...
    const reportTitle = reportPeriod ? `Sales Return Book (${reportPeriod})` : 'Sales Return Book';
    ...
    const exportColumns = useMemo<ReportExportColumn<SalesReturnBookRow>[]>(
        () => [
            { header: 'Return Id', value: (row) => row.saleReturnId },
            ...
            { header: 'Ref Invoice Ids', value: (row) => row.referenceInvoiceIds ?? '' },
            { header: 'Ref Amount', value: (row) => (row.referenceAmount ? row.referenceAmount.toFixed(2) : '') },
            { header: 'Cancelled', value: (row) => formatFlag(row.isCancelled) },
        ],
        []
    );

    const exportFileName = `sales-return-book_${fromDate ? toDateText(fromDate) : 'all'}_${toDate ? toDateText(toDate) : 'all'}`;
    ...
    const buildExportRows = useCallback(async () => {
        const rows = await fetchAllSalesReturnBookRows();
        return rows.filter((row) => !row.isSkeleton);
    }, [fetchAllSalesReturnBookRows]);
    ...
    const entryBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : row.saleReturnId ? String(row.saleReturnId) : '';

    const voucherBody = (row: SalesReturnBookRow) => {
    ...
    const ledgerAddressBody = (row: SalesReturnBookRow) => {
        if (isSkeletonRow(row)) {
    ...
    const remarksBody = (row: SalesReturnBookRow) => (isSkeletonRow(row) ? skeletonCell('8rem') : row.remarks ?? '');
    const amountBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalAmount);
    const taxBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalTaxAmount);
    const roundOffBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('3rem') : formatAmount(row.roundOffAmount);
    const netBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalNetAmount);
    const referenceIdsBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('6rem') : row.referenceInvoiceIds ?? '';
    const referenceAmountBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.referenceAmount);
    const cancelledBody = (row: SalesReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatFlag(row.isCancelled);
    const ledgerIdBody = (row: SalesReturnBookRow) =>
        ...
        <div className="card app-gradient-card">
            ...
            <div className="flex flex-column gap-2 mb-3 report-screen-header">
                <div className="flex flex-column md:flex-row md:align-items-start md:justify-content-between gap-3">
                    <div>
                        <h2 className="m-0">Sales Return Book</h2>
                        <p className="mt-2 mb-0 text-600">Sales return register with tax and net totals.</p>
                    </div>
                </div>
                {(reportError || ledgerError) && (
                    <p className="text-red-500 m-0">
                        Error loading sales return book: {reportError || ledgerError?.message}
                    </p>
                )}
            </div>
            ...
            <ReportDataTable
                ...
                size="small"
                loading={false}
                className="summary-table sales-return-book-table"
                emptyMessage={reportLoading ? '' : hasApplied ? 'No results found' : 'Press Refresh to load sales return book'}
                headerLeft={
                    <div className="flex flex-column gap-2 w-full">
                        <div className="flex align-items-center gap-2 flex-wrap">
                            <AppDateInput
                                ...
                                autoFocus
                                selectOnFocus
                                inputId="sales-return-book-from-date"
                            ...
                            <AppDateInput
                                ...
                                fiscalYearStart={fiscalRange?.start ?? null}
                                fiscalYearEnd={fiscalRange?.end ?? null}
                                inputId="sales-return-book-to-date"
                            ...
                            <div className="flex align-items-center gap-2">
                                <Checkbox
                                    inputId="sales-return-book-today"
                                ...
                                />
                                <label htmlFor="sales-return-book-today">Today</label>
                            </div>
                        </div>
                ...
                recordSummary={
                    hasApplied
                        ? reportLoading
                            ? 'Loading sales return book...'
                            : `${totalRecords} entry${totalRecords === 1 ? '' : 'ies'}`
                        : 'Press Refresh to load sales return book'
                }
            >
                <Column field="saleReturnId" header="Entry No." body={entryBody} style={{ width: '7rem' }} />
                <Column
                    header={
                ...
                <Column
                    ...
                    style={{ width: '8rem' }}
                />
                <Column field="referenceInvoiceIds" header="Ref Invoice" body={referenceIdsBody} style={{ width: '10rem' }} />
                <Column
                    field="referenceAmount"
                    header="Ref Amt"
                    body={referenceAmountBody}
                    headerClassName="summary-number"
                    bodyClassName="summary-number"
                    style={{ width: '8rem' }}
                />
