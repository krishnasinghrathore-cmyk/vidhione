...
type ReceiptForm = {
    ...
    chequeReceivedNo: string;
    chequeReceivedDate: Date | null;
    isFullPaid: boolean;
    discountAmount: number;
    chequeCancelCharges: number;
    isChequeCancelled: boolean;
    isChecked: boolean;
    remarks: string;
    lines: ReceiptLine[];
};
...
const MONEY_RECEIPT_BANK_BOOK = gql`
    query MoneyReceiptBankBook($input: MoneyReceiptBankBookInput) {
...
const CREATE_MONEY_RECEIPT_BANK = gql`
    mutation CreateMoneyReceiptBank($input: MoneyReceiptBankEntryInput!) {
        createMoneyReceiptBank(input: $input) {
            bankReceiptId
        }
    }
`;

const CANCEL_MONEY_RECEIPT_BANK = gql`
    mutation CancelMoneyReceiptBank($bankReceiptId: Int!, $cancelled: Boolean) {
        cancelMoneyReceiptBank(bankReceiptId: $bankReceiptId, cancelled: $cancelled)
    }
`;
...
export default function BillingMoneyReceiptBankPage() {
    ...
    const [chequeReceivedDateError, setChequeReceivedDateError] = useState<string | null>(null);
    const fromDateInputRef = useRef<HTMLInputElement>(null);
    const toDateInputRef = useRef<HTMLInputElement>(null);
    const refreshButtonId = 'money-receipt-bank-refresh';
    ...
    const { data: banksData } = useQuery(BANKS, { variables: { search: null, limit: 2000 } });
    const { data: branchesData } = useQuery(BRANCHES, { variables: { search: null, limit: 2000 } });
    ...
    const registerVariables = useMemo(
        ...
        [bankId, branchId, cancelled, fromDate, ledgerId, toDate]
    );
    ...
    const { data, loading, error, refetch } = useQuery(MONEY_RECEIPT_BANK_BOOK, {
        client: billingApolloClient,
        variables: appliedVariables ?? registerVariables,
        skip: !appliedVariables
    });

    const [createReceipt, { loading: saving }] = useMutation(CREATE_MONEY_RECEIPT_BANK, {
        client: billingApolloClient
    });
    const [setReceiptCancelled] = useMutation(CANCEL_MONEY_RECEIPT_BANK, { client: billingApolloClient });

    const rows = useMemo<MoneyReceiptBankRow[]>(() => (hasApplied ? data?.moneyReceiptBankBook?.items ?? [] : []), [data, hasApplied]);
    ...
    const bankOptions = useMemo(() => {
        const rows = (banksData?.banks ?? []) as BankRow[];
        return rows.map((row) => ({ label: row.name ?? `Bank ${row.bankId}`, value: row.bankId }));
    }, [banksData]);

    const branchOptions = useMemo(() => {
        const rows = (branchesData?.branches ?? []) as BranchRow[];
        return rows.map((row) => ({ label: row.name ?? `Branch ${row.branchId}`, value: row.branchId }));
    }, [branchesData]);
    ...
    const handleSave = async () => {
        ...
        if (form.chequeDate) {
            const chequeDateValidation = validateSingleDate({ date: form.chequeDate }, fiscalRange);
            if (!chequeDateValidation.ok) {
                ...
                toast.current?.show({ severity: 'warn', summary: 'Cheque date required', detail: message });
                return;
            }
        }
        if (form.chequeReceivedDate) {
            const receivedValidation = validateSingleDate({ date: form.chequeReceivedDate }, fiscalRange);
            if (!receivedValidation.ok) {
                const message = receivedValidation.errors.date ?? 'Cheque received date is required';
                setChequeReceivedDateError(message);
                toast.current?.show({ severity: 'warn', summary: 'Cheque received date required', detail: message });
                return;
            }
        }
        setVoucherDateError(null);
        setChequeDateError(null);
        setChequeReceivedDateError(null);
        ...
        if (!form.bankId) {
            toast.current?.show({ severity: 'warn', summary: 'Bank required', detail: 'Select a bank.' });
            return;
        }
            ...
            await createReceipt({
                variables: {
                    input: {
                        ...
                        chequeDateText: toDateText(form.chequeDate),
                        chequeReceivedNo: form.chequeReceivedNo?.trim() || null,
                        chequeReceivedDateText: toDateText(form.chequeReceivedDate),
                        ...
                        isChequeCancelled: form.isChequeCancelled,
    ...
    const handleCancel = async (row: MoneyReceiptBankRow) => {
        try {
            await setReceiptCancelled({
                variables: { bankReceiptId: row.bankReceiptId, cancelled: !row.isCancelled }
            });
            toast.current?.show({
                severity: 'success',
                summary: row.isCancelled ? 'Restored' : 'Cancelled',
                detail: `Receipt ${row.voucherNumber ?? row.bankReceiptId} updated.`
            });
            await refetch();
        } catch (err: any) {
    ...
    const statusBody = (row: MoneyReceiptBankRow) => (
        <Tag value={row.isCancelled ? 'Cancelled' : 'Active'} severity={row.isCancelled ? 'danger' : 'success'} />
    );

    const checkedBody = (row: MoneyReceiptBankRow) => (
        <Tag value={row.isChecked ? 'Checked' : 'Unchecked'} severity={row.isChecked ? 'info' : 'warning'} />
    );

    const chequeReturnBody = (row: MoneyReceiptBankRow) => {
        if (row.chequeReturnStatus) {
            return <Tag value={row.chequeReturnStatus} severity="danger" />;
        }
        ...
        <div className="card">
            ...
            <div className="flex flex-column gap-2 mb-3">
                <div className="flex flex-column md:flex-row md:align-items-start md:justify-content-between gap-3">
                    <div>
                        <h2 className="m-0">Money Receipt (Bank)</h2>
                        <p className="mt-2 mb-0 text-600">Create and manage bank receipts with invoice allocations.</p>
                    </div>
            ...
            >
                ...
                <Column field="bankName" header="Bank" style={{ minWidth: '160px' }} />
                <Column field="branchName" header="Branch" style={{ minWidth: '160px' }} />
                <Column field="chequeNo" header="Cheque No" style={{ minWidth: '140px' }} />
                <Column field="chequeDateText" header="Cheque Date" body={(row) => formatDate(row.chequeDateText)} />
                <Column
                    field="isFullPaid"
                    header="Full Paid"
                ...
                />
                <Column header="Return" body={chequeReturnBody} />
                ...
                <div className="grid">
                    ...
                    <div className="col-12 md:col-4">
                        ...
                        <AppDropdown
                            value={form.bankId}
                            options={bankOptions}
                            onChange={(e) => setForm((prev) => ({ ...prev, bankId: e.value }))}
                    ...
                    <div className="col-12 md:col-4">
                        ...
                        <AppDropdown
                            value={form.branchId}
                            options={branchOptions}
                            onChange={(e) => setForm((prev) => ({ ...prev, branchId: e.value }))}
                            placeholder="Optional"
                            showClear
                            filter
                            className="w-full"
                    ...
                    <div className="col-12 md:col-4">
                        ...
                        <AppDateInput
                            ...
                            onChange={(value) => {
                                setForm((prev) => ({ ...prev, chequeDate: value }));
                                setChequeDateError(null);
                            }}
                            fiscalYearStart={fiscalRange?.start ?? null}
                            fiscalYearEnd={fiscalRange?.end ?? null}
                    ...
                    <div className="col-12 md:col-4">
                        <label className="font-medium">Cheque Received No</label>
                        <InputText
                            value={form.chequeReceivedNo}
                            onChange={(e) => setForm((prev) => ({ ...prev, chequeReceivedNo: e.target.value }))}
                            className="w-full"
                        />
                    </div>
                    <div className="col-12 md:col-4">
                        <label className="font-medium">Cheque Received Date</label>
                        <AppDateInput
                            value={form.chequeReceivedDate}
                            onChange={(value) => {
                                setForm((prev) => ({ ...prev, chequeReceivedDate: value }));
                                setChequeReceivedDateError(null);
                            }}
                            fiscalYearStart={fiscalRange?.start ?? null}
                            fiscalYearEnd={fiscalRange?.end ?? null}
                            className="w-full"
                        />
                        {chequeReceivedDateError && <small className="text-red-500">{chequeReceivedDateError}</small>}
                    </div>
                    ...
                    <div className="col-12 md:col-4 flex align-items-center gap-2">
                        <Checkbox
                            inputId="chequeReturned"
                            checked={form.isChequeCancelled}
                            onChange={(e) => setForm((prev) => ({ ...prev, isChequeCancelled: !!e.checked }))}
                        />
                        <label htmlFor="chequeReturned" className="font-medium">
                            Cheque Returned
                        </label>
                    </div>
                    <div className="col-12 md:col-4 flex align-items-center gap-2">
