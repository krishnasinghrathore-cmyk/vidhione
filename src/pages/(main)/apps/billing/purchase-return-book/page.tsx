...
import type { PurchaseReturnBookPage } from '@/lib/invoicing/api';
...
type PurchaseReturnBookFilters = {
    fromDate: Date | null;
    toDate: Date | null;
    ledgerId: number | null;
...
export default function BillingPurchaseReturnBookPage() {
    ...
    const [appliedFilters, setAppliedFilters] = useState<PurchaseReturnBookFilters | null>(null);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [first, setFirst] = useState(0);
    const [loading, setLoading] = useState(false);
    const [reportError, setReportError] = useState<string | null>(null);
    const [reportData, setReportData] = useState<PurchaseReturnBookPage | null>(null);
    ...
    const refreshButtonId = 'purchase-return-book-refresh';

    useEffect(() => {
        setPageTitle('Purchase Return Book');
    ...
    const displayRows = useMemo<PurchaseReturnBookRow[]>(() => {
        return rawRows.map((row) => ({
            rowKey: String(row.purchaseReturnId),
    ...
    const buildQueryVariables = (filters: PurchaseReturnBookFilters, offset: number, limit: number) => ({
    ...
    const fetchAllPurchaseReturnBookRows = useCallback(async () => {
        if (!appliedFilters) return displayRows;
        if (totalRecords <= displayRows.length) return displayRows;
        try {
            const response = await invoicingApi.fetchPurchaseReturnBook(
    ...
    const { isPrinting, printRows, triggerPrint } = useReportPrint<PurchaseReturnBookRow>({
        rows: displayRows,
        getPrintRows: fetchAllPurchaseReturnBookRows
    });
    ...
    const buildFilters = (): PurchaseReturnBookFilters => ({
        fromDate,
        toDate,
        ledgerId,
    ...
    const loadPurchaseReturnBook = async (filters: PurchaseReturnBookFilters, offset: number, limit: number) => {
        setLoading(true);
        setReportError(null);
        try {
            const response = await invoicingApi.fetchPurchaseReturnBook(buildQueryVariables(filters, offset, limit));
            setReportData(response);
        } catch (err) {
            setReportError(err instanceof Error ? err.message : 'Failed to load purchase return book');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRefresh = () => {
        ...
        void loadPurchaseReturnBook(nextFilters, 0, rowsPerPage);
    };
    ...
    const handlePageChange = (event: DataTablePageEvent) => {
        ...
        void loadPurchaseReturnBook(appliedFilters, event.first, event.rows);
    };
    ...
    const reportTitle = reportPeriod ? `Purchase Return Book (${reportPeriod})` : 'Purchase Return Book';
    ...
    const exportColumns = useMemo<ReportExportColumn<PurchaseReturnBookRow>[]>(
        () => [
            { header: 'Return Id', value: (row) => row.purchaseReturnId },
    ...
    const exportFileName = `purchase-return-book_${fromDate ? toDateText(fromDate) : 'all'}_${toDate ? toDateText(toDate) : 'all'}`;
    ...
    const buildExportRows = useCallback(async () => {
        const rows = await fetchAllPurchaseReturnBookRows();
        return rows.filter((row) => !row.isSkeleton);
    }, [fetchAllPurchaseReturnBookRows]);
    ...
    const entryBody = (row: PurchaseReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : row.purchaseReturnId ? String(row.purchaseReturnId) : '';

    const voucherBody = (row: PurchaseReturnBookRow) => {
    ...
    const ledgerAddressBody = (row: PurchaseReturnBookRow) => {
        if (isSkeletonRow(row)) {
    ...
    const remarksBody = (row: PurchaseReturnBookRow) => (isSkeletonRow(row) ? skeletonCell('8rem') : row.remarks ?? '');
    const amountBody = (row: PurchaseReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalAmount);
    const taxBody = (row: PurchaseReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalTaxAmount);
    const netBody = (row: PurchaseReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatAmount(row.totalNetAmount);
    const cancelledBody = (row: PurchaseReturnBookRow) =>
        isSkeletonRow(row) ? skeletonCell('4rem') : formatFlag(row.isCancelled);
    const ledgerIdBody = (row: PurchaseReturnBookRow) =>
        ...
        <div className="card app-gradient-card">
            ...
            <div className="flex flex-column gap-2 mb-3 report-screen-header">
                <div className="flex flex-column md:flex-row md:align-items-start md:justify-content-between gap-3">
                    <div>
                        <h2 className="m-0">Purchase Return Book</h2>
                        <p className="mt-2 mb-0 text-600">Purchase return register with tax and net totals.</p>
                    </div>
                </div>
                {(reportError || ledgerError) && (
                    <p className="text-red-500 m-0">
                        Error loading purchase return book: {reportError || ledgerError?.message}
                    </p>
                )}
            </div>
            ...
            <ReportDataTable
                ...
                size="small"
                loading={false}
                className="summary-table purchase-return-book-table"
                emptyMessage={reportLoading ? '' : hasApplied ? 'No results found' : 'Press Refresh to load purchase return book'}
                headerLeft={
                    <div className="flex flex-column gap-2 w-full">
                        <div className="flex align-items-center gap-2 flex-wrap">
                            <AppDateInput
                                ...
                                autoFocus
                                selectOnFocus
                                inputId="purchase-return-book-from-date"
                            ...
                            <AppDateInput
                                ...
                                fiscalYearStart={fiscalRange?.start ?? null}
                                fiscalYearEnd={fiscalRange?.end ?? null}
                                inputId="purchase-return-book-to-date"
                            ...
                            <div className="flex align-items-center gap-2">
                                <Checkbox
                                    inputId="purchase-return-book-today"
                                ...
                                />
                                <label htmlFor="purchase-return-book-today">Today</label>
                            </div>
                        </div>
                ...
                recordSummary={
                    hasApplied
                        ? reportLoading
                            ? 'Loading purchase return book...'
                            : `${totalRecords} entry${totalRecords === 1 ? '' : 'ies'}`
                        : 'Press Refresh to load purchase return book'
                }
            >
                <Column field="purchaseReturnId" header="Entry No." body={entryBody} style={{ width: '7rem' }} />
                <Column
                    header={
