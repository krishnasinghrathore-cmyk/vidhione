...
export default function InventoryGodownsPage() {
    ...
    const { data, loading, error, refetch } = useQuery(GODOWNS, { client: inventoryApolloClient });
    ...
    const confirmDelete = (event: React.MouseEvent<HTMLButtonElement>, row: GodownRow) => {
        confirmPopup({
            target: event.currentTarget,
            message: 'Delete this godown?',
    ...
    return (
        <div className="card">
            ...
            <div className="flex flex-column gap-2 mb-3">
                <div className="flex flex-column md:flex-row md:align-items-start md:justify-content-between gap-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        <Button label="New Godown" icon="pi pi-plus" onClick={openNew} />
                    </div>
                </div>
            ...
            <AppDataTable
                ...
                size="small"
                loading={loading}
                onRowDoubleClick={(e) => openEdit(e.data as GodownRow)}
                headerLeft={
                ...
                headerRight={
                    <>
                        ...
                        <span className="text-600 text-sm">
                            Showing {filteredRows.length} godown{filteredRows.length === 1 ? '' : 's'}
                        </span>
                    </>
                }
                recordSummary={`${filteredRows.length} godown${filteredRows.length === 1 ? '' : 's'}`}
            >
