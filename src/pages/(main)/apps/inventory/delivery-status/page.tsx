...
const CREATE_DELIVERY_STATUS = gql`
...
const DELETE_DELIVERY_STATUS = gql`
    mutation DeleteDeliveryStatus($deliveryStatusId: Int!) {
        deleteDeliveryStatus(deliveryStatusId: $deliveryStatusId)
    }
`;
...
export default function InventoryDeliveryStatusPage() {
        ...
        try {
            ...
            if (editing) {
                await updateDeliveryStatus({
                    variables: {
                        deliveryStatusId: editing.deliveryStatusId,
                        ...variables
                    }
                });
            } else {
                await createDeliveryStatus({ variables });
            }
    ...
    const handleDelete = async (deliveryStatusId: number) => {
        try {
            await deleteDeliveryStatus({ variables: { deliveryStatusId } });
            await refetch();
            toastRef.current?.show({
                severity: 'success',
                summary: 'Deleted',
                detail: 'Delivery status deleted.'
            });
        } catch (e: any) {
            toastRef.current?.show({
                severity: 'error',
    ...
    const flagsBody = (row: DeliveryStatusRow) => {
    ...
    const colorBody = (row: DeliveryStatusRow) => {
        ...
        <div className="card">
            ...
            <div className="flex flex-column gap-2 mb-3">
                <div className="flex flex-column md:flex-row md:align-items-start md:justify-content-between gap-3">
                    <div>
                        <h2 className="m-0">Delivery Status</h2>
                    ...
                    <div className="col-12">
                        <div className="flex flex-wrap gap-4">
                            <span className="flex align-items-center gap-2">
                                ...
                                />
                                <label htmlFor="dsCompleted" className="text-sm text-600">
                                    Completed
                                </label>
                            </span>
                            <span className="flex align-items-center gap-2">
                                ...
                                />
                                <label htmlFor="dsRemark" className="text-sm text-600">
                                    Remark Required
                                </label>
                            </span>
                        </div>
